...
// GotchiFacet ABI with all required functions
const gotchiFacetABI = [
    // escrowBalance
    {
      "inputs": [
        { "internalType": "uint256", "name": "_tokenId", "type": "uint256" },
    ...
    // Add more GotchiFacet methods if needed
];

// Combine all facet ABIs into a single Diamond ABI
const diamondABI = [
    ...escrowFacetABI,
    ...gotchiFacetABI
    // Add more facet ABIs here if needed
];
...
async function connectWallet() {
  ...
  try {
    ...
    // Initialize contract with combined Diamond ABI
    contract = new ethers.Contract(contractAddress, diamondABI, signer);
...
function generateMethodForms() {
  ...
  for (const methodName in facetMethods) {
    ...
    // Create the toggle icon (downward triangle initially)
